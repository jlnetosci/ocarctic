...
#### FUNCTIONS ####
@st.experimental_dialog("Go to transcription")
def confirm_transcription():
    st.write("Please confirm you want to skip directly to transcription.")
    st.markdown("Note that refined selections yield better results. Please check our [tips](tips) section.", unsafe_allow_html=True)
    
    one, two, three = st.columns([4, 4, 2])
    with two:
...
st.info("**This is a preview of your selection. Please choose an option of the menu to proceed.**")
...
if 'cropped_img' not in st.session_state:
    ...
    if st.sidebar.button("Refine selection", use_container_width=True, key="refine_selection"):
