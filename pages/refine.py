...
## TITLE AND INFO ##
st.title("Refine selection")

st.info("**Refine the area of the image you want to transcribe. You will be able to preview your selection.**")
...
if 'cropped_img' not in st.session_state:
	...
	if st.session_state['cropped_img'].size[0] >= st.session_state['cropped_img'].size[1]:
		st.session_state['refined_selection'] = st_cropper(st.session_state['cropped_img'], box_color="#0f6787ff", stroke_width=5)

	else:
		one_of_three, two_of_three, three_of_three = st.columns([2, 20, 2])
	...
	st.sidebar.markdown(
		...
		<p style="text-align: justify;">
			Refine the area of interest and press:
		</p>
		""",
		unsafe_allow_html=True)

	if st.sidebar.button('Crop', use_container_width=True, key="sidebar_crop"):
		st.switch_page("pages/refined.py")
